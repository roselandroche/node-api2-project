...
- Type `npm install` to download all dependencies. DONE
- To start the server, type `npm run server` from the root folder (where the _package.json_ file is). The server is configured to restart automatically as you make changes. ***ALREADY CRASHED***
